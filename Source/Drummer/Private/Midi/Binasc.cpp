...
namespace smf
{
	...
	int Binasc::outputStyleAscii(std::ostream &out, std::istream &input)
	{
		...
		while (!input.eof())
		{
			...
			if ((type == 1) && (lastType == 0))
			{
				...
				if (index + lineCount >= m_maxLineLength)
				{ // put on next line
					outputWord[index] = '\0';
					out << '\n'
						<< outputWord;
							...
							int lineCount)
	{
		...
		while (i < length)
		{
			...
			else
			{
				...
				else if ((word.find(',') != std::string::npos) || (word.size() > 2))
				{
						...
						const std::string &terminators, int index)
	{
		...
		while (i < (int)input.size())
		{
			...
			if (escape && (i < (int)input.size() - 1) && (input[i] == '\\') && (input[i + 1] == '"'))
			{
	...
	int Binasc::outputStyleMidi(std::ostream &out, std::istream &input)
	{
		...
		if (ch != 'M')
		{
			std::cerr << "Not a MIDI file M" << std::endl;
			return 0;
		}
		input.read((char *)&ch, 1);
		if (ch != 'T')
		{
			std::cerr << "Not a MIDI file T" << std::endl;
			return 0;
		}
		input.read((char *)&ch, 1);
		if (ch != 'h')
		{
			std::cerr << "Not a MIDI file h" << std::endl;
			return 0;
		}
		input.read((char *)&ch, 1);
		if (ch != 'd')
		{
			std::cerr << "Not a MIDI file d" << std::endl;
			return 0;
		}
		tempout << "\"MThd\"";
		if (m_commentsQ)
		...
		for (i = 0; i < trackcount; i++)
		{
			...
			if (ch != 'M')
			{
				std::cerr << "Not a MIDI file M2" << std::endl;
				return 0;
			}
			input.read((char *)&ch, 1);
			if (ch != 'T')
			{
				std::cerr << "Not a MIDI file T2" << std::endl;
				return 0;
			}
			input.read((char *)&ch, 1);
			if (ch != 'r')
			{
				std::cerr << "Not a MIDI file r" << std::endl;
				return 0;
			}
			input.read((char *)&ch, 1);
			if (ch != 'k')
			{
				std::cerr << "Not a MIDI file k" << std::endl;
				return 0;
			}
			tempout << "\"MTrk\"";
			if (m_commentsQ)
